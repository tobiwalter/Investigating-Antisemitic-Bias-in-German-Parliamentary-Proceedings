...
class LabelPropagation:
  ...
  def propagate(self):
    ...
    L = sparse.csgraph.laplacian(self.ppmi_mat, normed=True).toarray()
  ...
  def save_scores(self, path):
    if not os.path.exists('f_u_scores'):
      os.makedirs('f_u_scores')
    np.save(f'f_u_scores/{path}', self.scores)
...
def main():
  ...
  labels = lp.create_labels(attributes[att_1], attributes[att_2])

  start = time.time()
  logging.info(f'Start label propagation')
  fu_scores = lp.propagate(labels)
  elapsed = time.time()
