...
class TWEC:
    ...
    def __init__(self, size=100, sg=0, siter=5, diter=5, ns=10, window=5, alpha=0.025,
        ...
        :param hs: Hierarchical softmax - If 1, hierarchical softmax will be used for model training (needed for testing slices)
    ...
    def train_model(self, sentences):
        print('New train_model')
        model = None
        if self.compass == None or self.init_mode != "copy":
            model = Word2Vec(sg=self.sg, size=self.size, alpha=self.static_alpha, iter=self.static_iter,hs=self.hs,
    ...
    def evaluate(self):
        ...
        for fn in sorted(mfiles):
            ...
            m = Word2Vec.load(fn)
            # m = KeyedVectors.load_word2vec_format(fn, binary=True)
            m.cbow_mean = True
        ...
        for n_tfn, tfn in enumerate(sorted(tfiles)):
            sentences = LineSentence(tfn, max_sentence_length= np.Inf)
